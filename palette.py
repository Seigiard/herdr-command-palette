...
Commands are read from ~/.config/herdr/command-palette/commands.toml (or
HERDR_COMMAND_PALETTE_CONFIG) plus sibling *.toml files. Entries can be global
or scoped to GitHub-style owner/repository identities in that one user-owned
file. For compatibility, project-local commands can also be read from
.herdr/command-palette/ under the pane's working directory. The palette
deliberately executes only commands from those trusted files; there is no
runtime command registration or shell parsing for Herdr argv entries.
"""
...
def normalize_repository(value: str) -> str:
    """Normalize common Git remote forms to owner/repository."""
...
def git_project_context(cwd: str) -> tuple[Path | None, str]:
    """Return the Git root and normalized origin repository for cwd."""
    if not cwd:
        return None, ""
    try:
    ...
    try:
        ...
        return root, ""
    repository = normalize_repository(remote_result.stdout) if remote_result.returncode == 0 else ""
...
def validate_repositories(raw: dict[str, Any], title: str, source: str) -> tuple[str, ...]:
    value = raw.get("repositories")
    if value is None:
        return ()
    if not isinstance(value, list) or not value:
        raise ValueError(f"command '{title}' ({source}) repositories must be a non-empty list of owner/repository strings")
    repositories: list[str] = []
    for entry in value:
        if not isinstance(entry, str) or not normalize_repository(entry):
            raise ValueError(f"command '{title}' ({source}) has invalid repository {entry!r}")
        repositories.append(normalize_repository(entry))
...
def validate_command_raw(raw: dict[str, Any], title: str, source: str) -> tuple[str, tuple[str, ...]]:
    ...
    validate_repositories(raw, title, source)
...
def load_commands_from_dir(
    ...
    for path in command_data_files(base_dir, main_file):
        ...
        for index, item in enumerate(items):
            ...
            repositories = validate_repositories(item, str(item.get("title") or item.get("name") or ""), source)
            if repositories and repository not in repositories:
                continue
            command_origin = "Project" if repositories else origin
            commands.append(command_from_raw(item, source, command_origin))
    return commands
...
def load_commands() -> tuple[Path, list[Command]]:
    ...
    project_dir = project_command_dir(target_cwd)
    if project_dir:
        commands.extend(load_commands_from_dir(project_dir, "Project", project_dir / "commands.toml", repository))
    global_commands = load_commands_from_dir(path.parent, "Global", path, repository)
    commands.extend(command for command in global_commands if command.origin == "Project")
    commands.extend(command for command in global_commands if command.origin == "Global")
...
def fzf_requirement_message(reason: str) -> str:
    ...
    return "\n".join(
        [
            ...
            "",
            "Install fzf 0.56 or newer with your package manager, for example:",
            "",
            "    brew install fzf",
...
def context_vars(config_path: Path, herdr: str | None = None) -> dict[str, str]:
    ...
    git_root, _ = git_project_context(target_cwd)
    local_command_dir = project_command_dir(target_cwd)
    project_root = str(git_root or (local_command_dir.parent.parent if local_command_dir else ""))
    return {
        "config_file": config,
