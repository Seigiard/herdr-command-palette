...
PALETTE_DIR="$PROJECT_ROOT"
REAL_COMMANDS="$PROJECT_ROOT/tests/fixtures/commands.toml"
...
setup() {
  ...
  export HERDR_COMMAND_PALETTE_CALL_TIMEOUT_SECONDS=10
...
function test_palette_004_palette_sources_compile() {
  _bats_test_init 4 'palette sources compile'
...
function test_palette_0041_test_timeout_override_does_not_change_the_default() {
  _bats_test_init 41 'the Herdr call timeout is overridable while the default stays two seconds'
  run env -u HERDR_COMMAND_PALETTE_CALL_TIMEOUT_SECONDS PALETTE_PY="$PALETTE_PY" python3 - <<'PY'
import importlib.util
import os
import sys

spec = importlib.util.spec_from_file_location("palette_default_timeout", os.environ["PALETTE_PY"])
module = importlib.util.module_from_spec(spec)
sys.modules[spec.name] = module
spec.loader.exec_module(module)
assert module.HERDR_CALL_TIMEOUT_SECONDS == 2, module.HERDR_CALL_TIMEOUT_SECONDS
PY
  assert_success

  run env HERDR_COMMAND_PALETTE_CALL_TIMEOUT_SECONDS=10 PALETTE_PY="$PALETTE_PY" python3 - <<'PY'
import importlib.util
import os
import sys

spec = importlib.util.spec_from_file_location("palette_test_timeout", os.environ["PALETTE_PY"])
module = importlib.util.module_from_spec(spec)
sys.modules[spec.name] = module
spec.loader.exec_module(module)
assert module.HERDR_CALL_TIMEOUT_SECONDS == 10, module.HERDR_CALL_TIMEOUT_SECONDS
PY
  assert_success
}
...
function test_palette_0151_one_global_file_scopes_commands_by_repository() {
  _bats_test_init 151 'one global file scopes commands by repository without repo-local config'
  local repository="$PALETTE_WORK/repository"
  local nested="$repository/packages/app"
  local plain="$PALETTE_WORK/plain"
  mkdir -p "$nested" "$plain"
  git init -q "$repository"
  git -C "$repository" remote add origin git@github.com:Seigiard/my-mac-setup.git
  cat > "$PALETTE_WORK/commands.toml" <<'TOML'
[[commands]]
title = "Global"
type = "shell"
...
TOML
  ...
  run env HERDR_TARGET_CWD="$nested" HERDR_COMMAND_PALETTE_CONFIG="$PALETTE_WORK/commands.toml" python3 - <<'PY'
import palette_boot

palette = palette_boot.palette()
config, commands = palette.load_commands()
assert [(command.title, command.origin) for command in commands] == [
    ("This repository", "Project"),
    ("Global", "Global"),
], commands
variables = palette.context_vars(config)
assert variables["project_root"].endswith("/repository"), variables
PY
  assert_success
  [ ! -e "$repository/.herdr" ]

  run env HERDR_TARGET_CWD="$plain" HERDR_COMMAND_PALETTE_CONFIG="$PALETTE_WORK/commands.toml" python3 - <<'PY'
import palette_boot

_, commands = palette_boot.palette().load_commands()
assert [command.title for command in commands] == ["Global"], commands
PY
  assert_success
}
...
function test_palette_054_r9_a_missing_fzf_fails_loudly_with_install_guidance() {
  _bats_test_init 54 'R9: a missing fzf fails loudly with install guidance'
...
PY
  ...
  run env PALETTE_LINK_JSON="$link_json" \
    HERDR_CONFIG="$PROJECT_ROOT/tests/fixtures/herdr/config.toml" python3 - <<'PY'
import json
import os
import re
